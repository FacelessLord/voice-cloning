...
def create_training_config():
    ...
    optimizer_g = torch.optim.AdamW(model.generator.parameters(), lr=5e-4, betas=(0.8, 0.99))
    optimizer_d = torch.optim.AdamW(model.discriminator.parameters(), lr=5e-4, betas=(0.8, 0.99))
    ...
    for epoch in range(config.num_epochs):
        ...
        for batch in progres_bar:
            ...
            progres_bar.set_postfix({
                "G_loss": g_loss.accept_data(losses['g_loss']),
                "D_loss": d_loss.accept_data(losses['d_loss']),
                "Mel_loss": mel_loss.accept_data(losses['mel_loss']),
                "Dur_loss": dur_loss.accept_data(losses['dur_loss']),
            })
